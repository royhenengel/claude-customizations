...
**Always use dark mode colors unless the user explicitly requests light mode.** This includes using the dark mode color palette for all diagram styling by default.
...
**Default: Use dark mode colors.** Only use light mode colors if the user explicitly requests light mode styling.
...
- **Using too many colors**: More than 5 colors creates visual chaos. Use a limited, semantic palette with 3-4 main colors with meaning
- **Pure black lines and text**: Too harsh, creates visual fatigue. Use dark gray for lines and text instead
- **Tiny text crammed into nodes**: Hard to read. Use concise labels with line breaks
- **Every box the same style**: No visual hierarchy. Highlight key components with color, size, or borders
- **Spaghetti of crossing lines**: Confusing flow. Use clean flow with subgraphs to group related items
- **Inconsistent styling**: Random colors, mixed fonts. Use consistent conventions where same colors mean same things
- **Trying to show everything**: Overwhelming complexity. Create multiple focused diagrams where each tells one story
...
**IMPORTANT**: Remember: A diagram should make something easier to understand. If it doesn't, it's not the right diagram or it needs better design. Don't be afraid to scrap and restart with a different approach.
