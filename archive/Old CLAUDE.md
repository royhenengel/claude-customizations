...
## Task Assessment (Automatic)

Before starting any task, assess these triggers:
...
**If NO triggers → Lightweight mode:**
- State: "Lightweight mode - [brief reason]"
- Use TodoWrite to track steps
- Execute in current conversation

**If ANY trigger → Structured mode:**
- State: "Structured mode - [which triggers hit]"
- Create .planning/PLAN.md with numbered tasks
- Execute each task via subagent (fresh context)
- Update TodoWrite after each task completes
- Create handoff file if stopping mid-work
