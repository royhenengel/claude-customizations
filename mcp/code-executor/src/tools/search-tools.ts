import type { SearchToolsInput, MCPConfig, ToolIndex } from "../types/index.js";
    ...
    try {
      ...
      if (connected) {
        ...
        if (server && server.status === "connected") {
          const tools: ToolIndex[] = server.tools.map(t => ({
            server: serverName,
            tool: t.name,
 ...
 */
async function lazyConnectServer(serverPool: ServerPool, serverName: string): Promise<boolean> {
